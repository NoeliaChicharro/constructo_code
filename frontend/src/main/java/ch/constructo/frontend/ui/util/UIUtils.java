...
public class UIUtils {
   ...
   */
  private static final ThreadLocal<DecimalFormat> numberFormat = ThreadLocal
      .withInitial(() -> new DecimalFormat("###,###,###"));

  private static final ThreadLocal<DecimalFormat> yearFormat = ThreadLocal
      .withInitial(() -> new DecimalFormat("####"));

  private static final ThreadLocal<DecimalFormat> decimalFormat = ThreadLocal
      .withInitial(() -> new DecimalFormat("###,###,###.00"));

  private static final ThreadLocal<DecimalFormat> intFormat = ThreadLocal
      .withInitial(() -> new DecimalFormat("###,###,###"));
  ...
  private static final ThreadLocal<DateTimeFormatter> dateCaseIdFormat = ThreadLocal
  ...
  /* === Noelia Labels === */
  public static Span createSimpleLabel(String text){
    Span span = new Span(text != null ? text : "  ");
    span.addClassName("span");
    return span;
  }

  public static Label createSimpleLabel1(String text){
    Label label = new Label(text != null ? text : "  ");
    label.getElement().getStyle().set("padding", "0 10px 0 0");
    return label;
  }

  public static Label createLabelWidth(String text){
    Label label = new Label(text != null ? text : " ");
    label.getElement().getStyle().set("minWidth", "15rem");
    label.getElement().getStyle().set("maxWidth", "15rem");
  ...
  public static String formatInt(long value) {
    return intFormat.get().format(value);
  }
  ...
  public static Label createVersionLabel(String version) {
    return createH5Label(version);
  }

  public static String formatMemory(long memory) {
    return decimalFormat.get().format(memory);
  }
  ...
  public static String formatDateTime(java.time.LocalDateTime date) {
    if(date==null){
      return " ";
    }
    return dateTimeFormatter.get().format(date);
  ...
  public static String formatYear(Integer year) {
    ...
    return yearFormat.get().format(year);
  }

  public static LocalDate convertToLocalDate(java.time.LocalDateTime dateTime){
    if(dateTime==null){
      return null;
    }
    return LocalDate.of(dateTime.getYear(), dateTime.getMonth(), dateTime.getDayOfMonth());
  }
  public static java.time.LocalDateTime convertToDateTime(LocalDate localDate){
    if(localDate==null){
      return null;
    }
  ...
  public static String formatDateToCaseId(LocalDate date) {
    return dateCaseIdFormat.get().format(date);
  }
...
// @formatter:on
  ...
  public static HorizontalLayout createProperty(String labelText, String value ){
    return  new HorizontalLayout(
        UIUtils.createLabelWidth(labelText),
        UIUtils.createSimpleLabel1(value)
    );
  }
